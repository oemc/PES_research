...
```
```{r}
vis_dat(datos_paper %>% filter(!iso2c %in% c('SS', 'ZW', 'BT', 'ER', 'GW', 'KP', 'LB', 'NG', 'PS', 'SO', 'VU', 'FM', 'KI', 'TL', 'CV', 'SB'),
               !year %in% c(1995, 1996, 1997, 1998, 1999, 2000, 2001, 2023)) %>%
          select(NY.GDP.PCAP.CN, NY.GDP.PCAP.CD, NY.GDP.MKTP.CD, NY.GDP.MKTP.CN)) 
  # NY.GDP.PCAP.CN, NY.GDP.PCAP.CD, NY.GDP.MKTP.CD, NY.GDP.MKTP.CN son buenos candidatos, 'SY'falta PIB per Capita en 2022
```
...
vis_dat(datos_paper %>% 
        filter(!iso2c %in% c('SS', 'ZW', 'BT', 'ER', 'GW', 'KP', 'LB', 'NG', 'PS', 'SO', 'VU', 'FM', 'KI', 'TL', 'CV', 'SB','SY'),
               !year %in% c(1995, 1996, 1997, 1998, 1999, 2000, 2001, 2023)) %>%
        select(iso2c, year, hdi, DT.ODA.ALLD.CD, DT.ODA.ALLD.KD, DT.ODA.ODAT.CD, DT.ODA.ODAT.KD, DT.ODA.ODAT.PC.ZS, NY.GDP.PCAP.CN, NY.GDP.PCAP.CD,
               all_of(gsub("_", ".", gob_indicators)), 
               ))
...
```
De 1925 observaciones reducimos a 1098 (hasta 2019) o a 
...
## Modelo HDI
Probando un modelo sencillo, regresion lineal, Minimos cuadrados, datos panel, HDI
```{r}
...
## Modelo GDP
Probando un modelo sencillo, regresion lineal, Minimos cuadrados, datos panel, GDP

```{r}
model_2 <- lm(NY.GDP.PCAP.CD ~ DT.ODA.ALLD.CD + CC.EST + GE.EST + PV.EST + RQ.EST + RL.EST, data=datos_model)
summary(model_2)
```
Government Effectiveness y Political Volatility son significativas al 99%
...
for (col in colnames(datos_model)[4:35]) {
...
```{r}
subplot(my_plot[6:7], nrows = 1, margin = 0.05) %>% layout(title = 'GDP vs HDI')
```

```{r}
subplot(my_plot[8:13], nrows = 2, margin = 0.05) %>% layout(title = 'CC vs HDI')
```

```{r}
subplot(my_plot[14:19], nrows = 2, margin = 0.05) %>% layout(title = 'GE vs HDI')
```


```{r}
subplot(my_plot[20:25], nrows = 2, margin = 0.05) %>% layout(title = 'PV vs HDI')
```

```{r}
subplot(my_plot[26:31], nrows = 2, margin = 0.05) %>% layout(title = 'RQ vs HDI')
```

```{r}
subplot(my_plot[32], nrows = 1, margin = 0.05) %>% layout(title = 'RL vs HDI')
```
...
datos_model_3 <- datos_model_2 %>%
  arrange(iso2c, year) %>%
  mutate(hdi_diff = hdi - lag(hdi), NY.GDP.PCAP.CN_diff = NY.GDP.PCAP.CN - lag(NY.GDP.PCAP.CN), NY.GDP.PCAP.CD_diff = NY.GDP.PCAP.CD - lag(NY.GDP.PCAP.CD)) %>%
  filter(!year %in% c(2001))

datos_model_3 %>% select(iso2c, year, hdi, hdi_diff)
datos_model_3 %>% select(iso2c, year, NY.GDP.PCAP.CD, NY.GDP.PCAP.CD_diff, NY.GDP.PCAP.CN, NY.GDP.PCAP.CN_diff)
...
for (col in colnames(datos_model_3)[4:38]) {
  my_plot_3[[col]] <- plot_ly(x = datos_model_3[[col]], y = datos_model_3[[36]],
...
```{r}
subplot(my_plot_3[6:7], nrows = 2, margin = 0.05) %>% layout(title = 'GDP vs HDI diff')
```

```{r}
subplot(my_plot_3[8:13], nrows = 2, margin = 0.05) %>% layout(title = 'CC vs HDI diff')
```

```{r}
subplot(my_plot_3[14:19], nrows = 2, margin = 0.05) %>% layout(title = 'GE vs HDI diff')
```

```{r}
subplot(my_plot_3[20:25], nrows = 2, margin = 0.05) %>% layout(title = 'PV vs HDI diff')
```

```{r}
subplot(my_plot_3[26:31], nrows = 2, margin = 0.05) %>% layout(title = 'RQ vs HDI diff')
```

```{r}
subplot(my_plot_3[32], nrows = 1, margin = 0.05) %>% layout(title = 'RL vs HDI diff')
```
...
summary(model_3)
```
## Modelo HDI_diff PER.RNK
Otro modelo ahora usando las variables PER.RNK en vez de EST
```{r}
...
## Modelo GDP_diff
```{r}
model_5 <- lm(NY.GDP.PCAP.CD ~ DT.ODA.ALLD.CD + CC.EST + GE.EST + PV.EST + RQ.EST + RL.EST, data=datos_model_3)
summary(model_5)
```
