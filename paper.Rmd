...
v = c('hdi', 'hdi_diff', 'NY.GDP.PCAP.CD', 'NY.GDP.PCAP.CD_diff', 'DT.ODA.ALLD.CD', 'DT.ODA.ALLD.CD_diff',
...
# Datos faltantes
for (v_ in v) {
  temp <- datos_paper_long %>% filter(variable == v_, is.na(value))
  if (temp %>% nrow() == 0) {
    next
  }
  for (i in 1:nrow(temp)) {
    if (temp[i,]$iso2c %in% delete_c) {
      next
    }
    if (temp[i,]$year %in% delete_y) {
      next
    }
    na_for_country = temp %>% filter(iso2c == temp[i,]$iso2c) %>% nrow()
    na_for_year    = temp %>% filter(year  == temp[i,]$year)  %>% nrow()
    print(paste(i, v_))
    if (na_for_country > na_for_year) {
      print(paste('se excluye:',temp[i,]$iso2c))
      delete_c = c(delete_c, temp[i,]$iso2c)
    }
    else {
      print(paste('se excluye:',temp[i,]$year))
      delete_y = c(delete_y, temp[i,]$year)
    }
  }
}

temp <- datos_paper_long %>% filter(variable %in% v, is.na(value))
while (temp %>% nrow() > 0) {
  candidato_country <- temp %>% group_by(iso2c) %>% summarise(count = n()) %>% slice_max(count) %>% slice(1)
  candidato_year <- temp %>% group_by(year) %>% summarise(count = n()) %>% slice_max(count) %>% slice(1)
  if (candidato_country$count > candidato_year$count) {
    delete_c = c(delete_c, candidato_country$iso2c)
    temp <- temp %>% filter(!iso2c == candidato_country$iso2c)
  }
  else {
    delete_y = c(delete_y, candidato_year$year)
    temp <- temp %>% filter(!year == candidato_year$year)
  }
}
...
datos_paper %>% 
  filter(!iso2c %in% delete_c, !year %in% delete_y) %>%
  select(all_of(v))
